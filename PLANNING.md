...
```
Relies on the manifest's `[[build]]` step (`cargo build --release`)
running on the user's machine. Requires a Rust toolchain locally. Good for
tracking `main` rather than a pinned release.
